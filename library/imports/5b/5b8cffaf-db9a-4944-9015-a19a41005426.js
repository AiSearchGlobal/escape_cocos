"use strict";
cc._RF.push(module, '5b8cf+v25pJRJAVoZpBAFQm', 'ViewModel');
// script/framework/component/ViewModel.ts

"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.VM = void 0;
var Log_1 = require("../../utils/Log");
var JsonOb_1 = require("../core/JsonOb");
var VM_EMIT_HEAD = 'VC:';
...
function setValueFromPath(obj, path, value, tag) {
    ...
    for (var i = 0; i < props.length; i++) {
        ...
        if (propName in obj === false) {
            Log_1.Log.error('[' + propName + '] not find in ' + tag + '.' + path);
            break;
        }
        if (i == props.length - 1) {
            obj[propName] = value;
...
var VMManager = /** @class */ (function () {
    ...
    VMManager.prototype.add = function (data, tag, activeRootObject) {
        ...
        if (tag.includes('.')) {
            Log_1.Log.error('cant write . in tag:', tag);
            return;
        }
        if (has) {
            Log_1.Log.error('already set VM tag:' + tag);
            return;
        }
    ...
    VMManager.prototype.get = function (tag) {
        ...
        if (res == null) {
            Log_1.Log.error('cant find VM from:', tag);
        }
        else {
            return res.vm;
        }
    ...
    VMManager.prototype.addValue = function (path, value) {
        ...
        var rs = path.split('.');
        if (rs.length < 2) {
            Log_1.Log.error('Cant find path:' + path);
        }
        ;
        var vm = this.get(rs[0]);
        if (!vm) {
            Log_1.Log.error('Cant Set VM:' + rs[0]);
            return;
        }
        ;
    ...
    VMManager.prototype.getValue = function (path, def) {
        ...
        var rs = path.split('.');
        if (rs.length < 2) {
            Log_1.Log.error('Get Value Cant find path:' + path);
            return;
        }
        ;
        var vm = this.get(rs[0]);
        if (!vm) {
            Log_1.Log.error('Cant Get VM:' + rs[0]);
            return;
        }
        ;
    ...
    VMManager.prototype.setValue = function (path, value) {
        ...
        var rs = path.split('.');
        if (rs.length < 2) {
            Log_1.Log.error('Set Value Cant find path:' + path);
            return;
        }
        ;
        var vm = this.get(rs[0]);
    ...
    VMManager.prototype.bindPath = function (path, callback, target, useCapture) {
        path = path.trim(); //防止空格,自动剔除
        if (path == '') {
            Log_1.Log.error(target.node.name, '节点绑定的路径为空');
            return;
        }
        if (path.split('.')[0] === '*') {
            Log_1.Log.error(path, '路径不合法,可能错误覆盖了 VMParent 的onLoad 方法, 或者父节点并未挂载 VMParent 相关的组件脚本');
            return;
        }
